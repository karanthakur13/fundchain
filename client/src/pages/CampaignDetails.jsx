...
const CampaignDetails = () => {
  ...
  const { getVolunteers, volunteer, donate, getDonations, contract, address } =
    useStateContext();
  ...
  const fetchVolunteers = async () => {
    const data = await getVolunteers(state.PId);
    setvolunteers(data);
  };
  ...
  const handleVolunteer = async () => {
    setIsLoading(true);
    console.log(v_name);
    await volunteer(state.PId, v_name);
    navigate("/");
    setIsLoading(false);
  };
      ...
      <div className="w-full flex md:flex-row flex-col mt-10 gap-[30px]">
        <div className="flex-1 flex-col">
          <img
            src={state.image}
            alt="campaign"
            className="w-full h-[537px] object-cover rounded-xl"
          />
        ...
        <div className="flex md:w-[150px] w-full flex-wrap justify-between gap-[30px]">
          ...
          <CountBox
            ...
            value={` ${state.fundRaised}`}
          />
          <CountBox title="Total Donators" value={donators.length} />
          <CountBox title="Total Volunteers" value={state.countVolunteer} />;
        </div>
      </div>
          ...
          <div>
            ...
            <div className="mt-[20px] flex flex-col gap-4">
              {volunteers.length > 0 ? (
                ...
                <p className="font-epilogue font-normal text-[16px] text-[#808191] leading-[26px] text-justify">
                  No volunteers yet. Volunteer now!
                </p>
              )}
            </div>
          </div>
          ...
          <div className="mt-[20px] flex flex-col p-4 bg-[#1c1c24] rounded-[10px]">
            ...
            <div>
              <div className="felx flex-col">
                ...
                <CustomButton
                  btnType="button"
                  title="Volunteer for Campaign"
                  styles="w-full bg-[#1dc071] mt-[20px]"
                  handleClick={handleVolunteer}
                />
              </div>
            </div>
          </div>
